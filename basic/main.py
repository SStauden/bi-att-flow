...
from basic.read_data import read_data, read_data_extended, get_squad_data_filter, update_config
...
def main_extended(config, train_set_path, test_set_path, train_set_name, test_set_name):
    set_dirs(config)
    with tf.device(config.device):
        if config.mode == 'train':
            _train_extended(config, train_set_path, test_set_path)
        elif config.mode == 'test':
            # _test(config)
            _test_extended(config, train_set_path, test_set_path, train_set_name, test_set_name)
...
def _train_extended(config, train_set_path, dev_set_path):
    data_filter = get_squad_data_filter(config)

    # TODO
    shared_path = train_set_path

    train_data = read_data_extended(config, 'train', config.load, data_filter=data_filter, data_path=train_set_path, shared_path=shared_path)
    dev_data = read_data_extended(config, 'dev', True, data_filter=data_filter, data_path=dev_set_path, shared_path=shared_path)
    ...
    # Variables - TODO Error appearing here
...
def _test_extended(config, train_set_path, test_set_path, train_set_name, test_set_name):

    # TODO
    shared_path = test_set_path

    test_data = read_data_extended(config, 'dev', True, data_path=test_set_path, shared_path=shared_path)
    ...
    # change directories for data dumping
    config.eval_dir = os.path.join("testing", "BiDAF-" + train_set_name, test_set_name, "eval")
    config.answer_dir = os.path.join("testing", "BiDAF-" + train_set_name, test_set_name, "answer")
    ...
    if config.dump_answer:
        print("dumping answer to {}".format(config.answer_dir))
        graph_handler.dump_answer(e)
    if config.dump_eval:
        print("dumping eval to {}".format(config.eval_dir))
        graph_handler.dump_eval(e)
