...
def train_bidaf(attack_name):

  train_set_path = attack_dict[attack_name]['train']
  test_set_path = attack_dict[attack_name]['test']

  print("Training BiDAF on " + attack_name)

  config = flags.FLAGS
  # config.out_dir = os.path.join(config.out_base_dir, config.model_name, str(config.run_id).zfill(2))
  config.out_dir = os.path.join("training_" + config.out_base_dir, attack_name)
  ...
  config.load = False
  main_extended(config, train_set_path, test_set_path, attack_name, attack_name)

if __name__ == "__main__":

  attack_name = sys.argv[1]
  train_bidaf(attack_name)
