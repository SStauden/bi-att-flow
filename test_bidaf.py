...
from basic.main import main_extended
...
def test_bidaf(train_set_name, test_set_name):

  train_set_path = attack_dict[train_set_name]['train']
  test_set_path = attack_dict[test_set_name]['test']
  
  print("Testing " + train_set_name + " BiDAF on " + test_set_name)

  config = flags.FLAGS

  # path for loading the model
  config.out_dir = os.path.join("training_" + config.out_base_dir, train_set_name)
  config.mode = 'test'
  # config.shared_path = '/nethome/sstauden/bi-att-flow/training_out/original/basic/shared.json'

  main_extended(config, train_set_path, test_set_path, train_set_name, test_set_name)

if __name__ == "__main__":

  train_set_name = sys.argv[1]
  test_set_name = sys.argv[2]
  test_bidaf(train_set_name, test_set_name)
